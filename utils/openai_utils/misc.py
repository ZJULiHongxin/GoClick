import re
import json
from utils.data_utils.misc import gemini_act_to_normal_act, claude_act_to_normal_act, qwen3vl_act_to_normal_act
...
def extract_gemini_cua_protocol_components(response):
    """<think>
    **Fixing the Momondo Search**

    Okay, so the user wants me to find a flight on Momondo.  Let's see... they need a one-way from Toronto (YYZ) to Vancouver (YVR) on December 15th, for three adults in economy.  Looking at the app... hmmm, it's *close*.  It's got the right date, destination (YVR), and passenger count (3), but it's showing the origin as "NYC" instead of "Toronto Pearson International." Not ideal.

    No problem, I can fix this.  Seeing the "back" arrow in the top left corner, I'll just tap that to go back a step.  That should bring me to the screen where I can edit the origin airport.  I'll change "NYC" to "Toronto Pearson International" and then re-initiate the search. It's currently on a loading screen, so I'll hit that back arrow, update the origin and try again. Hopefully, Momondo can pull up the right flights with this adjustment.

    </think>
    <think>
    Observation: The current screen shows that the app is "Searching for lower prices..." for a flight from NYC to YVR on December 15 for 3 people. The origin airport is incorrect; it should be Toronto Pearson International (YYZ), not NYC. I need to go back to the previous screen to correct the origin airport.

    Intent: I will click the back arrow at the top left of the screen to return to the flight search details page and correct the departure location.

    Outcome: The app will navigate back to the previous screen, allowing me to edit the flight search parameters, specifically the origin airport.
    </think>
    <action>
    {"name": "click_at", "args": {"y": 89, "x": 60}}
    </action>"""

    internal_thinking_content = response.split("<think>")[1].split("</think>")[0].strip()
    external_thinking_content = response.split("<think>")[-1].split("</think>")[0].strip()
    observation = response.split('bservation: ')[-1].split('\n')[0] if 'bservation: ' in response else ''
    progress = response.split('rogress: ')[-1].split('\n')[0] if 'rogress: ' in response else ''
    exception = response.split('xception: ')[-1].split('\n')[0] if 'xception: ' in response else ''
    decision = response.split('ecision: ')[-1].split('\n')[0] if 'ecision: ' in response else ''
    intent = response.split('ntent: ')[-1].split('\n')[0] if 'ntent: ' in response else ''
    prediction = response.split('utcome: ')[-1].split('\n')[0] if 'utcome: ' in response else ''

    action = response.split('<action>')[-1].split('</action>')[0].strip()

    normalized_action = gemini_act_to_normal_act(action)
    return {
        "internal_thinking_content": internal_thinking_content,
        "external_thinking_content": external_thinking_content,
        "observation": observation,
        "progress": progress,
        "exception": exception,
        "decision": decision,
...
def extract_claude_cua_protocol_components(response):
    """<think>
    **Fixing the Momondo Search**
    ...
    normalized_action = claude_act_to_normal_act(action)
    return {
        "internal_thinking_content": internal_thinking_content,
        "external_thinking_content": external_thinking_content,
        "observation": observation,
...
def extract_thought_components_qwen3vl(response):
    # <think>
    # To access the sports and science categories, I need to navigate away from the 'Top Stories' section. The menu icon at the top left is typically used for accessing different sections of the app.

    # </think>
    # Thought: Clicking the menu icon should reveal options to select specific categories like sports and science.
    # Action: Click the menu icon at the top left corner to explore more sections.
    # <tool_call>
    # {"name": "mobile_use", "arguments": {"action": "click", "coordinate": [57, 84]}}
    # </tool_call>"

    internal_thinking = response.split('<think>')[1].split('</think>')[0].strip()
    external_thinking = response.split('</think>')[-1].split('<tool_call>')[0].strip()
    raw_action = response.split('<tool_call>')[-1].split('</tool_call>')[0].strip()

    action = qwen3vl_act_to_normal_act(raw_action)
    return {
        "internal_thinking_content": internal_thinking,
        "external_thinking_content": external_thinking,
