...
def make_elemgnd_sample(task_id, text, loc, output_tag=WITHPOINT_TAG, format=None):
    query = random.choice(elemgnd_prompt).format(text=text)
    loc_str = reformat_loc_str(loc, format)
...
def make_textloc_sample(task_id, text, loc, output_tag='point', format=None):
...
# action space of the agent tasks for Google Gemini
GEMINI_CUA_ACT_DEF = {
    'answer': """Answer user's question: {"action_type": "answer", "text": "<answer_text>"}""",

    'status': """If you believe the task is complete, finish the task by using the "status" action with "successful" as goal status: {"name": "status", "args": {"goal_status": "successful"}}. If the task is infeasible (e.g., lack of information or inability to perform necessary actions), use: {"name": "status", "args": {"goal_status": "infeasible"}}""",

    'answer': """Provide the user with a faithful answer according to the task requirements and screenshot content: {"name": "answer", "args": {"text": "<answer_text>"}}""",

    'click_at': """Clicks at a specific coordinate on the screen: {"name": "click_at", "args": {"y": , "x": }}. The x and y values are based on a 1000x1000 grid and are scaled to the screen dimensions.""",

    'hover_at': """Hovers the mouse at a specific coordinate on the screen. Useful for revealing sub-menus: {"name": "hover_at", "args": {"y": , "x": }}. x and y are based on a 1000x1000 grid.""",
    
    'long_press_at': """Long-press at a specific screen coordinate: {"name": "long_press_at", "args": {"y": , "x": }}.""",

    'scroll_document': """Scrolls the entire screen "up", "down", "left", or "right": {"name": "scroll_document", "args": {"direction": str ("up", "down", "left", or "right")}}""",
    
    'scroll_at': """Scrolls a specific element or area at coordinate (x, y) in the specified direction by a certain magnitude: {"name": "scroll_at", "args": {"y": 500, "x": 500, "direction": "down", "magnitude": 400}}. Coordinates and magnitude (default 800) are based on a 1000x1000 grid. y: int (0-999), x: int (0-999), direction: str ("up", "down", "left", "right"), magnitude: int (0-999, Optional, default 800)""",

    'type_text_at': """Types text at a specific coordinate: {"name": "type_text_at", "args": {"y": , "x": , "text": "...", "press_enter": true/false}}. Thsi action defaults to clearing the field first and pressing ENTER after typing, but these can be disabled. x and y are based on a 1000x1000 grid.""",

    'key_combination': """Press keyboard keys or combinations, such as "Control+A", "Control+C", or "Enter": {"name": "key_combination", "args": {"keys": "..."}}. Useful for triggering actions (like submitting a form with "Enter") or clipboard operations.""",

    'go_back': """Navigates to the previous screen: {"name": "go_back", "args": {}}. You can use this action instead of clicking a back button on the screen.""",

    'go_home': """Navigates to the home screen: {"name": "go_home", "args": {}}""",

    'wait_5_seconds': """Waits for 5 seconds: {"name": "wait_5_seconds", "args": {}}""",
    
    'open_app': """Opens an app (nothing will happen if the app is not installed): {"name": "open_app", "args": {"app_name": "..."}}. (Important: Using open_app is more efficient than swiping up to open the app drawer.)""",
}

# action space of the agent tasks for Google Claude
CLAUDE_CUA_ACT_DEF = {
    'answer': """Answer user's question: {"action_type": "answer", "text": "<answer_text>"}""",
    ...
    'left_click': """Clicks at a specific coordinate on the screen: {"action": "left_click", "coordinate": [x, y]}""",

    'hover': """Hovers the mouse at a specific coordinate on the screen. Useful for revealing sub-menus: {"action": "hover", "coordinate": [x, y]}""",
    
    'long_press': """Long-press at a specific screen coordinate: {"action": "long_press", "coordinate": [x, y]}""",

    'scroll': """Scrolls the entire screen "up", "down", "left", or "right": {"action": "scroll", "scroll_direction": str ("up", "down", "left", or "right"), "scroll_amount": ...}""",
    
    'scroll_at': """Scrolls a specific element or area at coordinate (x, y) in the specified direction by a certain magnitude: {"action": "scroll_at", "coordinate": [x, y], "scroll_direction": "down", "scroll_amount": ...}""",

    'type': """Types text at a specific coordinate: {"action": "type", "coordinate": [x, y], "text": "...", "press_enter": true/false}""",

    'key': """Press keyboard keys or combinations, such as "Control+A", "Control+C", or "Enter": {"action": "key", "keys": "..."}. Useful for triggering actions (like submitting a form with "Enter") or clipboard operations.""",

    'navigate_back': """Navigates to the previous screen: {"action": "navigate_back"}. You can use this action instead of clicking a back button on the screen.""",

    'navigate_home': """Navigates to the home screen: {"action": "navigate_home"}""",

    'wait': """Waits to pause between actions: {"action": "wait", "time": (integer, in seconds)}""",
    
    'open_app': """Opens an app (nothing will happen if the app is not installed): {"action": "open_app", "app_name": "..."}. (Important: Using this action is more efficient than swiping up to open the app drawer.)""",
}
...
To paste text into a text box, first long press the text box, then the text selection bar will appear with a `paste` button in it.""",
    ...
    'scale1000': """All coordinates are in the range of 0-1000."""
}
...
ACT_SPACE = {
    ...
    'AndroidWorld-stepcopilot': ['answer', 'status_successful', 'status_infeasible', 'click_xy1000', 'long-press_xy', 'swipe_xy', 'input-text_xy', 'open_app', 'navigate_back', 'navigate_home', 'press_enter', 'wait']
}

# https://ai.google.dev/gemini-api/docs/computer-use?hl=zh-cn#supported-actions
GEMINI_CUA_ACT_SPACE = {
    'all': ['status_successful', 'click_at', 'long_press_at ', 'scroll_document', 'type_text_at', 'key_combination', 'go_back', 'go_home', 'wait_5_seconds', 'open_app'],
    
    'unified_mobile': ['status_successful', 'click_at', 'long_press_at ', 'scroll_document', 'type_text_at', 'key_combination', 'go_back', 'go_home', 'wait_5_seconds', 'open_app'],
    
    'AITW': ['status_successful', 'click_at', 'scroll_document', 'type_text_at', 'key_combination', 'go_back', 'go_home'],
    'AndroidControl': ['click_at', 'scroll_document', 'type_text_at',  'go_back', 'wait_5_seconds', 'open_app'],
    'AndroidWorld': ['answer', 'status_successful', 'status_infeasible', 'click_at', 'long_press_at ', 'scroll_at', 'type_text_at', 'open_app', 'go_back', 'go_home', 'key_combination', 'wait_5_seconds'],
    'AndroidControl_reflec': ['click_at', 'scroll_at', 'type_text_at',  'go_back', 'wait_5_seconds', 'open_app'],
    'StepCopilot': ['click_xy', 'scroll_at', 'type_text_at',  'go_back', 'wait_5_seconds', 'open_app'],
    'AndroidWorld-stepcopilot': ['answer', 'status_successful', 'status_infeasible', 'click_at', 'long_press_at ', 'scroll_at', 'type_text_at', 'open_app', 'go_back', 'go_home', 'key_combination', 'wait_5_seconds']
}


# https://docs.claude.com/en/docs/agents-and-tools/tool-use/computer-use-tool
CLAUDE_CUA_ACT_SPACE = {
    'all': ['status_successful', 'left_click', 'hover', 'long_press', 'scroll', 'type', 'key', 'navigate_back', 'navigate_home', 'wait', 'open_app'],
    
    'unified_mobile': ['status_successful', 'left_click', 'hover', 'long_press', 'scroll', 'type', 'key', 'navigate_back', 'navigate_home', 'wait', 'open_app'],
    
    'AITW': ['status_successful', 'left_click', 'scroll', 'type', 'key', 'navigate_back', 'navigate_home'],
    'AndroidControl': ['left_click', 'scroll', 'type', 'navigate_back', 'wait', 'open_app'],
    'AndroidWorld': ['answer', 'status_successful', 'status_infeasible', 'left_click', 'hover', 'long_press', 'scroll', 'type', 'open_app', 'navigate_back', 'navigate_home', 'key', 'wait'],
    'AndroidControl_reflec': ['left_click', 'scroll', 'type',  'navigate_back', 'wait', 'open_app'],
    'StepCopilot': ['left_click', 'scroll', 'type',  'navigate_back', 'wait', 'open_app'],
    'AndroidWorld-stepcopilot': ['answer', 'status_successful', 'status_infeasible', 'left_click', 'hover', 'long_press', 'scroll', 'type', 'open_app', 'navigate_back', 'navigate_home', 'key', 'wait']
}
...
GUIDELINES = {
    ...
    'AndroidWorld-stepcopilot': ['scale1000']
}
...
# Qwen3: https://github.com/QwenLM/Qwen3-VL/blob/main/cookbooks/mobile_agent.ipynb
Qwen3VL_SYS_PROMPT = """
...
Qwen3VL_SYS_PROMPT_FORMAT_REQUIREMENTS = """Response format for every step:
1) Thought: one concise sentence explaining the next move (no multi-step reasoning).
2) Action: a short imperative describing what to do in the UI.
3) A single <tool_call>...</tool_call> block containing only the JSON: {"name": <function-name>, "arguments": <args-json
-object>}.                                                                                                              
Rules:
...
Qwen3VL_SYS_PROMPT_FORMAT_REQUIREMENTS_OUTCOME_PRED = """Response format for every step:
1) Thought and Outcome: explaining the next move and predicting the outcome of the move. If you find that the initially chosen move cannot advance towards task completion, replan, explain, and predict the outcome of the new move until you believe the latest move can advance towards task completion.
2) Action: a short imperative describing what to do in the UI.
3) A single <tool_call>...</tool_call> block containing only the JSON: {"name": <function-name>, "arguments": <args-json
-object>}.                                                                                                              
Rules:
- Output exactly in the order: Thought and Outcome, Action, <tool_call>.
- The Thought and Outcome content should contain a detailed reasonging process.
- Do not output anything else outside those three parts.
- If finishing, use action=terminate in the tool call."""

Qwen3VL_QUERY_TEMPLATE = "The user query: {instruction}.\nTask progress (You have done the following operation on the current device): {history}.\n"

# DOUBAO Seed
DOUBAO_SYS_PROMPT = """You are a GUI agent. You are given a task and your action history, with screenshots. You need to perform the next action to complete the task.
...
COMPRESSED_STRUCTURED_PROTOCOL_V2 = """(Observe the current GUI screen, break down the task into manageable step-by-step subtasks while tracking progress through action history, select the most appropriate action plan, and (Important!) finally predict the detailed outcome of taking the selected action at the end of this reasoning.)"""
...
INTENT_PROTOCOL = """(Describe your action intent in Chinese before planning the next action.)"""

GUI_GUIDELINES = """- There are usually multiple ways to complete a task; select the simplest option. If an action doesn't work as expected, consider retrying it or switching to another method if you see from the history that it has failed previously.
- If the desired state is already achieved (e.g., enabling Wi-Fi when it's already on), complete the task.
...
# For Google Gemini
GEMINI_CUA_PLANNING_PROMPT_PROTOCOL = """You are an agent that can operate a {device} to complete the user's tasks. At each step, based on the current screenshot and action history, you must perform one of the actions below to advance toward task completion.
{action_space}
{guidelines}
The user's task is: {global_task}
Action history: {history}
...
<action>
(Output the planned action in the format of the defined funciton call above.)
</action>"""
...
def make_claude_cua_action_space(bmk_name: str):
    """Generate the action space description based on benchmark name and model type.
    ...
    Returns:
        String describing the available actions
    """
    acts = deepcopy(CLAUDE_CUA_ACT_SPACE[bmk_name])

    selected_actions = [CLAUDE_CUA_ACT_DEF[k] for k in acts]
...
def make_gemini_cua_action_space(bmk_name: str):
    """Generate the action space description based on benchmark name and model type.
    ...
    Returns:
        String describing the available actions
    """
    acts = deepcopy(GEMINI_CUA_ACT_SPACE[bmk_name])

    selected_actions = [GEMINI_CUA_ACT_DEF[k] for k in acts]
...
def make_guidelines(bmk_name: str, guidelines_mode: str = 'base+extra'):
    extra_guidelines = '\n'.join(f'- {TASK_GUIDELINES[k]}' for k in GUIDELINES[bmk_name])
    
    guide = 'Useful guidelines to follow:\n'
    if guidelines_mode == 'base+extra':
        guide += GUI_GUIDELINES.format(extra_guidelines=extra_guidelines)
    elif guidelines_mode == 'base':
        guide += extra_guidelines
...
PROTOCOL_DICT = {
    ...
    'simple-wFunc': COMPRESSED_STRUCTURED_PROTOCOL_V2,
    'direct': DUMMY_PROTOCOL,
    'intent_zh': INTENT_PROTOCOL,
...
def make_claude_cua_protocol(bmk_name: str, task: str = '', history: str = '', elem_list_str: str = '', device_type: str = 'digital device', protocol_type: str = 'v3', guidelines_mode: str = '', use_obs: bool = True, use_progress: bool = True, use_intent: bool = True, use_outcome: bool = True, **kwargs: Any):
    action_space = make_claude_cua_action_space(bmk_name)
    guide = make_guidelines(bmk_name, guidelines_mode) if guidelines_mode else ''
    ...
    prompt = GEMINI_CUA_PLANNING_PROMPT_PROTOCOL.format(
        device=device_type,
        action_space=action_space,
        global_task=task,
        history=history,
...
def make_gemini_cua_protocol(bmk_name: str, task: str = '', history: str = '', elem_list_str: str = '', device_type: str = 'digital device', protocol_type: str = 'v3', guidelines_mode: str = '', use_obs: bool = True, use_progress: bool = True, use_intent: bool = True, use_outcome: bool = True, **kwargs: Any):
    action_space = make_gemini_cua_action_space(bmk_name)
    guide = make_guidelines(bmk_name, guidelines_mode) if guidelines_mode else ''
...
def make_planning_protocol(bmk_name: str, task: str = '', history: str = '', elem_list_str: str = '', device_type: str = 'digital device', use_unnorm_xy: bool = False, use_index: bool = False, use_square_xy_brackets: bool = False, protocol_type: str = 'v3', guidelines_mode: str = '', use_qwen_actspace: bool = False, use_obs: bool = True, use_progress: bool = True, use_intent: bool = True, use_outcome: bool = True, input_text_with_coord: bool = True):
...
REASONING_REQUIREMENTS = {
    'Observation': 'Observe the content on the given GUI screenshot and then analyze the task progress according to the action history.',
    ...
    'Intent': 'Summarize the most appropriate action plan you select to take in a single sentence.',
    'Outcome': 'Predict the outcome of the action. You need to consider the functionality of the target element if the action requires interacting with an element.'
}
...
def make_som_planning_prompt(bmk_name: str, task: str, history: str, elem_list_str: str = '', device_type: str = 'digital device', use_unnorm_xy: bool = False, use_index: bool = True, use_square_xy_brackets: bool = False, use_guidelines: bool = False, actspace_type: str = 'qwen2p5', use_obs: bool = True, use_progress: bool = True, use_intent: bool = True, use_outcome: bool = True):
    ...
    if use_outcome:
        reason_req.append(f"Outcome: {REASONING_REQUIREMENTS['Outcome']}")
    reasoning_requirements = '\n'.join(reason_req)
    prompt_base = PLANNING_PROMPT_SOM
...
# Referring Expressions for 'navigate_recent', which displays recently used apps
NAVIGATE_RECENT_PREFIXES = [
    "Show the recent apps.",
    "Open the app switcher.",
    "Go to recent applications.",
    "Display my recently used apps.",
    "Pull up the recent menu.",
    "Take me to the multitasking screen.",
    "Switch to my recent tasks.",
    "View recent apps."
    ]
...
HOVER_TEMPLATE_QWEN = '{{"action": "hover", "coordinate": [{target_x}, {target_y}]}}'
...
def qwen2p5_to_original_action(action: dict | str, w: int, h: int, use_norm: bool = False):
    ...
    if (coordinate := action.get('coordinate', None)):
        target = [coordinate[0] / w, coordinate[1] / h] if use_norm else coordinate
    ...
    if action_type == 'click':
        ...
        norm_from_x, norm_from_y = [start[0] / w, start[1] / h] if use_norm else start
        norm_to_x, norm_to_y = [end[0] / w, end[1] / h] if use_norm else end
        ...
        return DRAG_TEMPLATE.format(start_x=norm_from_x, start_y=norm_from_y, end_x=norm_to_x, end_y=norm_to_y)
            ...
            return INPUT_TEMPLATE.format(text=action.get('text', action.get('value')))
    ...
    elif action_type in ['open_app', 'open']:
        return OPEN_APP_TEMPLATE.format(app_name=action.get('text', action.get('app_name', '')))
    elif action_type == 'wait':
...
# Annotate CoT
COT_ANNO_PROMPT_PROTOCOL = """You are an expert AI agent designed to rationalize the decision-making process for completing tasks on a digital interface. Your primary function is to generate a step-by-step rationale that logically leads to a predetermined "ground truth action."

Based on the provided context (current screen, user goal, and action history), you will explain the reasoning for taking the given Ground Truth Action.

Crucially, you must frame your entire response as if you are generating the action plan yourself. Do not, under any circumstances, reveal that you were provided with the correct action in advance. Your output should read like a proactive decision, not a retroactive explanation.
...
Your output must strictly follow this step-by-step format:
<structured_thinking>
{reasoning_requirements}
</structured_thinking>
<coherent_thinking>
...
def make_cot_anno_protocol(bmk_name: str, ground_truth_action: str, task: str = '', history: str = '', elem_list_str: str = '', device_type: str = 'digital device', use_unnorm_xy: bool = False, use_index: bool = False, use_square_xy_brackets: bool = False, protocol_type: str = 'v3', guidelines_mode: str = '', use_qwen_actspace: bool = False, use_obs: bool = True, use_progress: bool = True, use_intent: bool = True, use_outcome: bool = True, actspace_type: str = 'qwen2p5'):
    if actspace_type is not None and actspace_type.strip():
        action_space = make_action_space(bmk_name, actspace_type=='qwen2p5', use_unnorm_xy, use_index, use_square_xy_brackets) + '\n'
    else:
        action_space = ''
    ...
    prompt = COT_ANNO_PROMPT_PROTOCOL.format(
        device=device_type,
        action_space=action_space,
        global_task=task,
        history=history,
...
# 20250907: v2: Added division limit (less than 10)
ANNO_PROMPT_V2_EN_ZH = """You are an expert UI/UX analyst. Your task is to analyze a graphical user interface (GUI) screenshot and identify all **functional regions**. A functional region is a high-level grouping of individual UI elements (such as buttons, input fields, links, navigation elements, or icons) that collectively serve a single, overarching purpose.

**Instructions:**

1. **Identify All First-Level Functional Regions:** Scrutinize the entire GUI to identify every distinct, interactive region, including navigation bars, user profile management region, search regions, detailed content regions, sidebars, app windows, and other interactive regions.
2. **Determine Functionality:** For each identified region, first describe its visual layout and then deduce its primary functionality in detail.
    2.1. Provide a high-level description of the region/element's function. Avoid detailing every specific functionality. Instead, focus on its broader impact on the webpage experience. For example, if interacting with a "Products" region reveals a dropdown menu, do not catalog the subsequent webpage changes in exhaustive detail.
...
3. **Generate Normalized Bounding Boxes:** For each region, provide the precise bounding box coordinates in a normalized format `[x_min, y_min, x_max, y_max]`. The coordinates must be between 0 and 1000, where `(x_min, y_min)` is the top-left corner and `(x_max, y_max)` is the bottom-right corner of the region. All boxes should tightly bound the region and MUST not overlap.
4. **Be Thorough:** Identify all non-overlapping regions. Skip purely decorative or non-interactive elements like static text, background images, or simple separators.
5. **Division Limit:** The number of the detected functional regions should be less than 10 as too many regions may indicate that you accidentally break down a large region into too many small regions.
6. **Determine Dividability:** For each region, determine whether it can be divided into smaller lower-level self-contained functional regions. If yes, set the "dividable" field to true; if no, set it to false.
7. **Classify the types of the functional regions according to the following dictionary:**
{
    "Primary Interface Containers": {
        "Entire GUI": "Only used for the overall GUI item",
        "Application Window": "Main container for a desktop application, including window controls and all internal UI.",
...
8. **Adhere to the Output Format Below:** Provide the final output as a single JSON array. Please first describe the overall GUI and then the detected functional regions in fine detail. Do not hallucinate non-existing visual contents. Set the "type" field of the overall GUI item to "Entire GUI".
Output Format:
...
[
  {
    "bbox": [0, 0, 1000, 1000],
    "dividable": (true means this region can be divided into smaller self-contained functional regions, false means this region cannot be divided anymore),
    "type": (The type of the functional region),
    "description_en": "(Region layout and appearance description in English)",
    "functionality_en": "(Region's contextual functionality description in English)",
  },
  ...
]
```

If the screenshot represents an individual element, just mention the overall GUI as it cannot be divided into smaller regions.

6. Here are 4 examples
Example 1: Given a screenshot of VS Code running on Ubuntu, you can output the following JSON:
[
  {
    "bbox": [0, 0, 1000, 1000],
    "dividable": true,
    "type": "Entire GUI",
    "description_en": "A screenshot of a Ubuntu desktop environment, with the Visual Studio Code application open as the main window. The application is in dark mode and is displaying the settings page.",
    "description_zh": "一个桌面环境的屏幕截图，很可能是Linux发行版上的GNOME，其中Visual Studio Code应用程序作为主窗口打开。该应用程序处于暗黑模式，并显示设置页面。",
    "functionality_en": "The user interface provides access to the operating system's features and applications, with a primary focus on code editing and project management within the Visual Studio Code editor.",
    "functionality_zh": "该用户界面提供对操作系统功能和应用程序的访问，主要侧重于在Visual Studio Code编辑器中进行代码编辑和项目管理。"
  },
  {
    "bbox": [231, 197, 766, 946],
    "dividable": true,
    "type": "Application Window",
    "description_en": "A prominent rectangular window located in the center of the desktop, with a distinct dark theme and a menu bar at the top.",
    "description_zh": "一个位于桌面中央的矩形窗口，具有独特的深色主题和顶部的菜单栏。",
    "functionality_en": "This is the primary application window for Visual Studio Code, providing an integrated development environment for editing code, managing project files, and accessing development tools.",
    "functionality_zh": "这是 Visual Studio Code 的主应用程序窗口，提供一个用于编辑代码、管理项目文件和访问开发工具的集成开发环境。"
  },
  {
    "bbox": [0, 0, 1000, 22],
    "dividable": true (Because this top bar comprises these functional regions: Activities Button, Active Application Menu, Clock and Notification Center, and System Status Menu),
    "type": "Header / Top Bar",
    "description_en": "A horizontal bar that spans the entire width of the screen at the very top, featuring a black background and white text.",
    "description_zh": "一个位于屏幕顶部的水平条，横跨整个屏幕宽度，具有黑色背景和白色文本。",
    "functionality_en": "This top bar provides essential system-wide information and controls, including the current time and date, system status indicators, and access to system-level settings and notifications.",
    "functionality_zh": "这个顶部栏提供重要的系统级信息和控制，包括当前时间和日期、系统状态指示器以及对系统设置和通知的访问。"
  },
  {
    "bbox": [0, 22, 31, 1000],
    "dividable": false (Because this dock only contains undividable elements, which do not form a functional region),
    "type": "Sidebar / Side Navigation",
    "description_en": "A vertical, semi-transparent bar located on the left side of the screen, containing a series of application icons.",
    "description_zh": "一个位于屏幕左侧的垂直半透明栏，包含一系列应用程序图标。",
    "functionality_en": "This vertical dock serves as the application launcher for the Ubuntu operating system, providing a convenient way for users to quickly launch and switch between their favorite and running applications.",
    "functionality_zh": "这个垂直停靠栏是 Ubuntu 操作系统的应用程序启动器，为用户提供一种快速启动和切换他们收藏和正在运行的应用程序的便捷方式。"
  }
]

Example 2: Given a screenshot of MS Excel, with a modal pop-up window titled "Move/Copy Sheet" centered on the screen and being the only interactive region, you can output:
[
  {
    "bbox": [0, 0, 1000, 1000],
    "dividable": true,
    "type": "Entire GUI",
    "description_en": "The user interface of the Excel, showing a spreadsheet titled \"NGSS Assessment Resource Hyperlink\". A \"Move/Copy Sheet\" modal dialog is open and centered, partially obscuring the spreadsheet data.",
    "description_zh": "Excel应用程序的用户界面，显示一个名为“NGSS 评估资源超链接”的电子表格。一个“移动/复制工作表”模态对话框已打开并居中，部分遮挡了电子表格数据。",
    "functionality_en": "This interface allows for the creation, editing, and management of spreadsheet data. The currently active dialog provides functionality to reorganize worksheets within the workbook.",
    "functionality_zh": "此界面允许创建、编辑和管理电子表格数据。当前活动的对话框提供在工作簿内重组工作表的功能。"
  },
  {
    "bbox": [390, 240, 608, 706],
    "dividable": true (Because this dialog box comprises Move/Copy Sheet Button, Destination Folder Button, and New Name Input Field),
    "type": "Modal / Dialog Box",
    "description_en": "A small, rectangular pop-up window centered on the main application interface, with a title bar labeled 'Move/Copy Sheet' at the top.",
    "description_zh": "一个位于主应用程序界面中央的小型矩形弹出窗口，顶部有标题栏，标有'移动/复制工作表'。",
    "functionality_en": "This dialog box allows users to move or copy a selected sheet within the current Excel spreadsheet, providing options to choose the action, destination, and new name for the sheet.",
    "functionality_zh": "此对话框允许用户在当前的Excel电子表格中移动或复制选定的工作表，提供选择操作、目标位置和工作表新名称的选项。"
  }
]

Example 3: Given a screenshot of a single search bar element on YouTube, you can output:
[
  {
    "bbox": [0, 0, 1000, 1000],
    "dividable": false,
    "type": "Entire GUI",
    "description_en": "A horizontally elongated, pill-shaped input field with a light gray background. It contains the placeholder text 'Search' and a magnifying glass icon on the right side, indicating the search function.",
    "description_zh": "一个水平拉长的药丸形输入框，背景为浅灰色。它包含占位符文本“搜索”和右侧的放大镜图标，表示搜索功能。",
    "functionality_en": "This search bar is the primary tool for users to find and discover video content on the YouTube platform. Users can type keywords, phrases, or video titles to search for relevant videos, channels, or playlists.",
    "functionality_zh": "该搜索栏是用户在 YouTube 平台上查找和发现视频内容的主要工具。用户可以输入关键字、词组或视频标题来搜索相关的视频、频道或播放列表。"
  }
]

Example 4: Given an incomplete or corrupted screenshot of a functional region that cannot be processed, you can output an empty JSON array:
[]

I have given you a {is_dividable}GUI screenshot{context}.
Now it's your turn.
"""


ANNO_PROMPT_V2_EN = """You are an expert UI/UX analyst. Your task is to analyze a graphical user interface (GUI) screenshot and identify all **functional regions**. A functional region is a high-level grouping of individual UI elements (such as buttons, input fields, links, navigation elements, or icons) that collectively serve a single, overarching purpose.
...
7. **Classify the types of the functional regions according to the following dictionary:**
{
    ...
    "Others (You should come up with standard type names you think are most suitable for the region)": {
        "Others (Come up with a sub-type name, too)": "Other functional regions that you believe are a better fit but do not fall into the above categories."
    }
}
8. **Adhere to the Output Format Below:** Provide the final output as a single JSON array. Please first describe the overall GUI and then the detected functional regions in fine detail. Do not hallucinate non-existing visual contents. Set the "type" field of the overall GUI item to "Entire GUI".
Output Format:
...
[
  {
    "id": ".../10" (The maximum number of regions is 10),
    "bbox": [0, 0, 1000, 1000],
    "dividable": (true means this region can be divided into smaller self-contained functional regions, false means this region cannot be divided anymore),
    "type": (The type of the functional region),
    "description": "(Region layout and appearance description in English)",
    "functionality": "(Region's contextual functionality description in English)"
  },
  ...
]
```
...
[
  {
    ...
    "dividable": true,
    "type": "Entire GUI",
    "description": "A screenshot of a Ubuntu desktop environment, with the Visual Studio Code application open as the main window. The application is in dark mode and is displaying the settings page.",
    "functionality": "The user interface provides access to the operating system's features and applications, with a primary focus on code editing and project management within the Visual Studio Code editor."
  },
  {
    "id": "2/10",
    "bbox": [231, 197, 766, 946],
    "dividable": true,
    "type": "Application Window",
    "description": "A prominent rectangular window located in the center of the desktop, with a distinct dark theme and a menu bar at the top.",
    "functionality": "This is the primary application window for Visual Studio Code, providing an integrated development environment for editing code, managing project files, and accessing development tools."
  },
  {
    "id": "3/10",
    "bbox": [0, 0, 1000, 22],
    "dividable": true (Because this top bar comprises these functional regions: Activities Button, Active Application Menu, Clock and Notification Center, and System Status Menu),
    "type": "Header / Top Bar",
    "description": "A horizontal bar that spans the entire width of the screen at the very top, featuring a black background and white text.",
    "functionality": "This top bar provides essential system-wide information and controls, including the current time and date, system status indicators, and access to system-level settings and notifications."
  },
  {
    "id": "4/10",
    "bbox": [0, 22, 31, 1000],
    "dividable": false (Because this dock only contains undividable elements, which do not form a functional region),
    "type": "Sidebar / Side Navigation",
    "description": "A vertical, semi-transparent bar located on the left side of the screen, containing a series of application icons.",
    "functionality": "This vertical dock serves as the application launcher for the Ubuntu operating system, providing a convenient way for users to quickly launch and switch between their favorite and running applications."
  }
]
...
[
  {
    ...
    "dividable": true,
    "type": "Entire GUI",
    "description": "The user interface of the Excel, showing a spreadsheet titled \"NGSS Assessment Resource Hyperlink\". A \"Move/Copy Sheet\" modal dialog is open and centered, partially obscuring the spreadsheet data.",
    "functionality": "This interface allows for the creation, editing, and management of spreadsheet data. The currently active dialog provides functionality to reorganize worksheets within the workbook."
  },
  {
    "id": "2/10",
    "bbox": [390, 240, 608, 706],
    "dividable": true (Because this dialog box comprises Move/Copy Sheet Button, Destination Folder Button, and New Name Input Field),
    "type": "Modal / Dialog Box",
    "description": "A small, rectangular pop-up window centered on the main application interface, with a title bar labeled 'Move/Copy Sheet' at the top.",
    "functionality": "This dialog box allows users to move or copy a selected sheet within the current Excel spreadsheet, providing options to choose the action, destination, and new name for the sheet."
  }
]
...
[
  {
    ...
    "dividable": false,
    "type": "Entire GUI",
    "description": "A horizontally elongated, pill-shaped input field with a light gray background. It contains the placeholder text 'Search' and a magnifying glass icon on the right side, indicating the search function.",
    "functionality": "This search bar is the primary tool for users to find and discover video content on the YouTube platform. Users can type keywords, phrases, or video titles to search for relevant videos, channels, or playlists."
  }
]
...
CHECK_REGION_COMPLETENESS_PROMPT = """You are a meticulous AI assistant specializing in visual analysis and quality control. Your task is to evaluate a cropped image depicting a functional region within a larger GUI screenshot and determine the completeness and boundedness of the shown region, providing a quantifiable score.

Useful Guidelines:
A functional region is a high-level grouping of individual UI elements (such as buttons or input fields) that collectively serve a single, overarching purpose. In images with multiple overlapping windows, the primary application window in focus should be considered the main functional region.

A region box is considered valid if it meets both of the following criteria:
- Completeness: This measures how much of the functional region is visible and unobstructed within the provided boundaries. A region is considered incomplete if its key components are obscured or occluded by other overlapping elements (like pop-up dialogs or other windows). This score is independent of the box's framing.

- Boundedness: This measures the quality of the framing. A region is poorly bounded if the box is either: (a) Too tight: The box crops, clips, or cuts off any part of the functional region.
Too loose: (b) The box includes excessive, non-functional padding or background area that is not part of the subject.


Your Task
Analyze the user-provided image and its highlighted region. Then, provide your assessment in the specified format.

Scoring Rubrics
A. Completeness Score (Evaluates Occlusion):

3 (Perfect): The functional region is fully visible within the box. No part of it is occluded by other elements.
2 (Almost Complete): Only minor, non-critical parts of the region are occluded.
1 (Mostly Incomplete): A large or critical part of the region is occluded.
0 (Severely Incomplete): The vast majority of the functional region is occluded or missing.

B. Boundedness (Evaluates Framing): (a) Yes: The box tightly frames the entire functional region without cropping it and without including excessive padding. (b) No: The box either crops the functional region or includes excessive padding.

Critical Rule: If a region is cropped by its boundary (Boundedness: No), its Completeness Score cannot be 3, because a cropped view is inherently an incomplete one.

Instructive Examples:
Example 1: Perfect Case
(Image: A single application window perfectly filling the screenshot from edge to edge.)

Reasoning: The region is perfectly framed without being cropped, and its content is fully visible.
Completeness Score: 3
Boundedness: Yes

Example 2: Poor Boundedness (Excessive Padding)
(Image: A small calculator application window shown in the middle of a large, empty desktop.)

Reasoning: The region is fully visible, but it is poorly bounded due to excessive empty padding around it.
Completeness Score: 3
Boundedness: No

Example 3: Poor Boundedness (Cropping)
(Image: A web browser window where the right side and bottom are clearly cut off by the edge of the screenshot.)

Reasoning: The region is poorly bounded because it is cropped by the image frame, making the view incomplete.
Completeness Score: 1
Boundedness: No

Example 4: Poor Completeness (Occlusion)
(Image: A perfectly framed file explorer window, but a "Save As" dialog box is covering the central list of files.)

Reasoning: The region is well-bounded, but its completeness is low because a critical area is occluded by a dialog box.
Completeness Score: 1
Boundedness: Yes

Example 5: Poor Completeness AND Poor Boundedness
(Image: An image of VSCode, which is both occluded by a browser and cropped at the bottom.)

Reasoning: The region is poorly bounded because it is cropped, and its completeness is low due to significant occlusion from the overlapping browser window.
Completeness Score: 1
Boundedness: No


Output Format:
Reasoning: [Provide a brief, one-sentence explanation for the score, referencing incompleteness, excessive padding, or perfect framing.]
Completeness Score: [0-3]
Boundedness: [Yes/No]

I have given you an image of a functional region.{context_info}
Now it's your turn.
"""