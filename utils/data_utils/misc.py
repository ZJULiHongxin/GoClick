import cv2, re, random, json, os, math, magic, megfile
import unicodedata
import numpy as np
...
def resize_pil_image(img, max_size=1008):
...
def pred_2_point(pred, keep_box=True, scale=1000, w=None, h=None):
    ...
    if scale == -1:
        scaled_click_point = [click_point[0] / w, click_point[1] / h, click_point[2] / w, click_point[3] / h] if len(click_point) == 4 else  [click_point[0] / w, click_point[1] / h]
    elif any(x > 1 for x in click_point):
        scaled_click_point = [x / scale for x in click_point]
    else:
        scaled_click_point = click_point

    return scaled_click_point
...
def load_image(img_file: str, tonumpy: bool = True) -> Image.Image:
    if 's3://' in img_file:
...
def remove_trailing_commas(json_string: str) -> str:
    """
    Removes commas immediately followed by a closing curly brace '}'
    or square bracket ']', optionally separated by whitespace.

    These trailing commas cause errors in strict JSON parsers.

    Args:
        json_string: The potentially invalid JSON string containing trailing commas.

    Returns:
        The cleaned JSON string suitable for parsing.
    """
    # 1. Handle trailing commas before '}' (for JSON objects)
    # Pattern: Match a comma (,), followed by zero or more whitespace (\s*),
    # followed by a closing curly brace (\}).
    # We replace the entire match with just the closing brace.
    cleaned_string = re.sub(r',\s*\}', '}', json_string)

    # 2. Handle trailing commas before ']' (for JSON arrays, as a robust fix)
    # Pattern: Match a comma (,), followed by zero or more whitespace (\s*),
    # followed by a closing square bracket (\]).
    # We replace the entire match with just the closing bracket.
    cleaned_string = re.sub(r',\s*\]', ']', cleaned_string)

    return cleaned_string

# A classic example of finding "connected components." The most efficient way to solve this is by using a Disjoint Set Union (DSU) data structure, often called Union-Find.
class UnionFind:
    """
    An efficient Disjoint Set Union (DSU) data structure.
    
    It uses path compression and union by size/rank (implicitly here)
    to achieve near-constant-time operations.
    """
    def __init__(self):
        # Stores the parent of each item.
    ...
    def find(self, i):
        ...
        """
        # If i is not in the map, it is its own parent (a new set)
        if i not in self.parent:
            self.parent[i] = i
            return i
        
        # Path compression:
        # If i is not its own parent, recursively find the root
        # and set i's parent directly to the root.
        if self.parent[i] != i:
    ...
    def union(self, i, j):
        ...
        if root_i != root_j:
            # Make the root of i's set the parent of the root of j's set
            self.parent[root_j] = root_i
            return True # Indicates a merge happened
        return False # They were already in the same set

def merge_lists_with_shared_elements(lists):
    """
    Merges sublists that share one or more common elements.

    Args:
        lists: A list of lists (e.g., [[1, 2], [2, 3], [4, 5]])

    Returns:
        A new list of lists where all connected sublists are merged
    ...
    # Pass 1: Union elements within each sublist
    for sublist in lists:
        if not sublist:
            continue
        
        # Add all elements to our tracking set
        all_elements.update(sublist)
        
        # Get the first element to union all others against
        first_element = sublist[0]
        for i in range(1, len(sublist)):
            uf.union(first_element, sublist[i])

    # Pass 2: Group all elements by their root
    # A defaultdict(set) is perfect for this
    ...
    # Convert the groups (which are sets) into lists
    return [list(group) for group in groups.values()]


def gemini_act_to_normal_act(raw_action: str | Dict) -> Dict:
    """
    Convert a Gemini action to a normal action.
    """
    if isinstance(raw_action, str):
        raw_action = json.loads(raw_action)
    
    act_name = raw_action['name']
    if act_name == 'click_at':
        return {
            'action_type': 'click',
            'target': (raw_action['args']['x'], raw_action['args']['y'])
        }
    elif act_name == 'hover_at':
        return {
            'action_type': 'hover',
            'target': (raw_action['args']['x'], raw_action['args']['y'])
        }
    elif act_name == 'long_press_at':
        return {
            'action_type': 'long_press',
            'target': (raw_action['args']['x'], raw_action['args']['y'])
        }
    elif act_name == 'scroll_document':
        swipe_direction = scroll2swipe(raw_action['args']['direction'])
        return {
            'action_type': 'swipe', 'direction': swipe_direction
        }
    elif act_name == 'scroll_at': # {"action_type": "swipe", "direction": <"up", "down", "left", "right">, "start": (x, y)}
        swipe_direction = scroll2swipe(raw_action['args']['direction'])
        return {
            'action_type': 'swipe',
            'direction': swipe_direction,
            'start': (raw_action['args']['x'], raw_action['args']['y'])
        }
    elif act_name == 'type_text_at':
        return {
            'action_type': 'input_text',
            'text': raw_action['args']['text'],
            'target': (raw_action['args']['x'], raw_action['args']['y'])
        }
    elif act_name == 'key_combination':
        key_str = raw_action['args']['keys']
        if 'enter' in key_str:
            return {'action_type': 'press_enter'}
        else:
            return {
                'action_type': 'key_combination',
                'keys': raw_action['args']['keys']
            }
    elif act_name == 'go_back':
        return {
            'action_type': 'navigate_back'
        }
    elif act_name == 'go_home':
        return {
            'action_type': 'navigate_home',
        }
    elif act_name == 'status':
        return {
            'action_type': 'status',
            'goal_status': raw_action['args']['goal_status']
        }
        ...
        return {
            ...
            'app_name': raw_action['args']['app_name']
        }

def claude_act_to_normal_act(raw_action: str | Dict) -> Dict:
    """
    Convert a Claude action to a normal action.
    """
    if isinstance(raw_action, str):
        raw_action = json.loads(raw_action)
    
    act_name = raw_action['action']
    if act_name == 'left_click':
        return {
            'action_type': 'click',
            'target': raw_action['coordinate']
        }
    elif act_name == 'hover':
        return {
            'action_type': 'hover',
        ...
        swipe_direction = scroll2swipe(raw_action['scroll_direction'])
        return {
            'action_type': 'swipe', 'scroll_direction': swipe_direction
        }
    elif act_name == 'scroll_at': # {"action_type": "swipe", "scroll_direction": <"up", "down", "left", "right">, "start": (x, y)}
        swipe_direction = scroll2swipe(raw_action['scroll_direction'])
        return {
            'action_type': 'swipe',
            'scroll_direction': swipe_direction,
            'start': raw_action['coordinate']
        }
    elif act_name == 'type':
        return {
            'action_type': 'input_text',
            'text': raw_action['text'],
            'coordinate': raw_action['coordinate']
        }
    elif act_name == 'key':
        key_str = raw_action['keys']
        if 'enter' in key_str:
            return {'action_type': 'press_enter'}
        else:
            return {
                'action_type': 'key_combination',
                'keys': raw_action['keys']
            }
    elif act_name == 'navigate_back':
        return {
            'action_type': 'navigate_back'
        }
    elif act_name == 'navigate_home':
        return {
            'action_type': 'navigate_home',
        }
    elif act_name == 'status':
        return {
            'action_type': 'status',
            'goal_status': raw_action['goal_status']
        }
    elif act_name == 'answer':
        return {
            'action_type': 'answer',
        ...
        return {
            'action_type': 'open_app',
            'app_name': raw_action['app_name']
        }

def qwen3vl_act_to_normal_act(raw_action: str | Dict) -> Dict:
    """
    Convert a Qwen3VL action to a normal action.
    """
    if isinstance(raw_action, str):
        raw_action = json.loads(raw_action)
    
    args = raw_action['arguments']
    act_name = args['action']
    if act_name == 'click':
        return {
            'action_type': 'click',
            'target': args['coordinate']
        }
    elif act_name == 'hover':
        return {
            'action_type': 'hover',
        ...
        swipe_direction, swipe_distance = get_swipe_direction(args['coordinate'], args['coordinate2'])
        return {
            'action_type': 'swipe',
            'direction': swipe_direction,
            'start': args['coordinate']
        }
    elif act_name == 'type':
        return {
            'action_type': 'input_text',
    ...
    elif act_name == 'system_button':
        if args['button'].lower() == 'back':
            return {
                'action_type': 'navigate_back'
            }
        elif args['button'].lower() == 'home':
            return {
                'action_type': 'navigate_home',
            }
        elif args['button'].lower() == 'enter':
            return {
                'action_type': 'press_enter'
            }
        elif args['button'].lower() == 'menu':
            return {
                'action_type': 'navigate_recent'
            }
    elif act_name == 'terminate':
        return {
            'action_type': 'status',
            'goal_status': args['status']
        }
    elif act_name == 'answer':
        return {
            'action_type': 'answer',
